module draw_ghost
   ...
   always_ff @(posedge clk, negedge rst_n) begin
       if (~rst_n) count <= 'd0;
       else if (collided_g) count <= 'd0;
       else if (inter_count == 32'hFFFFFFF) count <= count + 1;
   end


   always_ff @(posedge clk, negedge rst_n) begin
       if (~rst_n) reg_collided_g <= 1'b0;
       else if (collided_g) reg_collided_g <= 1'b1;
       else if (inter_count >= 32'hFFFFFFF) reg_collided_g <= 1'b0;
   end
   ...
   always_comb begin
       ...
       case (currState)
           LEFT: begin
               if (collision_g && ~reg_collided_g) begin
                   nextState = RIGHT;
                   en_right = 1'b1;
               end
   ...
   always_ff @(posedge clk, negedge rst_n) begin
       if (~rst_n) begin
           ...
           if ((nghost_x > (141 + player_size)) && (nghost_x < (774 - player_size))) begin
               ghost_x <= nghost_x;
           end
           if ((nghost_y > (30 + player_size)) && (nghost_y < (504 - player_size))) begin
               ghost_y <= nghost_y;
           end
       end
   end
